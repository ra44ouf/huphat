    ...
    async function fetchBooks() {
      try {
        ...
        const { data, error } = await supabase
          .from("books")
          .select(`*, profiles (display_name_ar, display_name_en, is_verified)`)
    ...
    <main className="min-h-screen bg-background flex flex-col">
      ...
      <section className="bg-shubuhat-green py-20 px-4 relative overflow-hidden">
        <div className="absolute inset-0 opacity-10 pointer-events-none select-none overflow-hidden">
          <div className="text-[20vw] font-black absolute -top-10 -left-10 text-white leading-none">
            BOOKS
          </div>
        </div>
        <div className="max-w-6xl mx-auto text-center text-white relative z-10">
          ...
          <div className="inline-flex bg-white/10 backdrop-blur-sm rounded-full p-1.5 border border-white/20">
            <button
              ...
              className={`px-6 py-3 rounded-full font-black text-sm transition-all duration-300
                ${activeTab === "books"
                  ? "bg-shubuhat-gold text-shubuhat-green shadow-lg"
                  : "text-white/70 hover:text-white"}`}
            >
              {lang === "ar" ? "📚 الكتب العلمية" : "📚 Books"}
            </button>
            <button
      ...
      {activeTab === "books" && (
        <section className="max-w-6xl mx-auto px-4 py-20 w-full">
          {/* Search bar */}
          <div className="max-w-xl mx-auto flex items-center bg-white border border-shubuhat-border-lite rounded-full p-1 shadow-sm overflow-hidden group mb-12">
          ...
          {loading ? (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="aspect-[3/4] bg-shubuhat-green-ghost rounded-[40px] animate-pulse" />
              ))}
            </div>
          ) : books.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-shubuhat-text-3 font-bold">
                {lang === "ar"
                  ? "انتظرونا، سيتم إضافة كتب قيمة قريباً"
                  : "Valuable books will be added soon"}
              </p>
            </div>
          ) : (() => {
                  ...
                  <div key={book.id} className="group flex flex-col">
                    ...
                    <div className="px-2">
                      ...
                      <div className="flex items-center gap-2 mt-4 text-[11px] font-bold text-shubuhat-text-3">
                        ...
                        <span>
                          {lang === "ar"
                            ? book.profiles?.display_name_ar
                            : book.profiles?.display_name_en}
                        </span>
